...
BidirectionalLstmRnn::BidirectionalLstmRnn(MatrixGenerator *W_init,
	...
	dL_dx_z_f(new GpuMatrix(p_init->nrows, kMaxSentenceLen)), dL_dx_z_b(new GpuMatrix(p_init->nrows, kMaxSentenceLen)),
	dL_dx_i_f(new GpuMatrix(p_init->nrows, kMaxSentenceLen)), dL_dx_i_b(new GpuMatrix(p_init->nrows, kMaxSentenceLen)),
	dL_dx_f_f(new GpuMatrix(p_init->nrows, kMaxSentenceLen)), dL_dx_f_b(new GpuMatrix(p_init->nrows, kMaxSentenceLen)),
	dL_dx_o_f(new GpuMatrix(p_init->nrows, kMaxSentenceLen)), dL_dx(new GpuMatrix(p_init->nrows, kMaxSentenceLen)) {
...
BidirectionalLstmRnn::~BidirectionalLstmRnn(void) {
	...
	delete dL_dx_z_f; delete dL_dx_z_b;
	delete dL_dx_i_f; delete dL_dx_i_b;
	delete dL_dx_f_f; delete dL_dx_f_b;
	delete dL_dx_o_f; delete dL_dx;
...
float BidirectionalLstmRnn::forwardPropagation(GpuMatrix *sentence) {
	...
	Wz_f->dot(sentence, &GpuMatrix(pre_z_f->data, input_dim, n), z_context_f);
	Wi_f->dot(sentence, &GpuMatrix(pre_z_b->data, input_dim, n), i_context_f);
	Wo_f->dot(sentence, &GpuMatrix(pre_i_f->data, input_dim, n), o_context_f);
	Wz_b->dot(sentence, &GpuMatrix(pre_i_b->data, input_dim, n), z_context_b);
	Wi_b->dot(sentence, &GpuMatrix(pre_o_f->data, input_dim, n), i_context_b);
	Wo_f->dot(sentence, &GpuMatrix(pre_o_b->data, input_dim, n), o_context_b);

	forward_lstm_block[0]->forwardPropagation(pre_z_f_columns[0], pre_i_f_columns[0], pre_o_f_columns[0], zero_vector, zero_vector);
	backward_lstm_block[0]->forwardPropagation(pre_z_b_columns[0], pre_i_b_columns[0], pre_o_b_columns[0], zero_vector, zero_vector);
	for (int t = 1; t < n; t++) {
		forward_lstm_block[t]->forwardPropagation(pre_z_f_columns[t], pre_i_f_columns[t], pre_o_f_columns[t], forward_lstm_block[t-1]->h_t, forward_lstm_block[t-1]->c_t);
		backward_lstm_block[t]->forwardPropagation(pre_z_f_columns[n-1-t], pre_i_f_columns[n-1-t], pre_o_f_columns[n-1-t], backward_lstm_block[t-1]->h_t, backward_lstm_block[t-1]->c_t);
	}

	float pre_exp = forward_lstm_block[n-1]->h_t->vdot(w_hy_f, o_context_f) +
			        backward_lstm_block[n-1]->h_t->vdot(w_hy_b, o_context_b);
...
void BidirectionalLstmRnn::backwardPropagation(GpuMatrix *sentence, int sentence_grammaticality) {
	...
	for (int t = 0; t < n; t++) {
		...
		if (t == 0) {
			forward_lstm_block[n-1]->h_t->scale(error, dL_dh_f, o_context_f);
			backward_lstm_block[n-1]->h_t->scale(error, dL_dh_b, o_context_b);
	...
	// here should be submatrixes created
	Wz_f->dot(true, dL_dpre_z_f, dL_dx_z_f, z_context_f);
	Wi_f->dot(true, dL_dpre_i_f, dL_dx_i_f, i_context_f);
	Wf_f->dot(true, dL_dpre_f_f, dL_dx_f_f, f_context_f);
	Wo_f->dot(true, dL_dpre_o_f, dL_dx_o_f, o_context_f);
	Wz_b->dot(true, dL_dpre_z_b, dL_dx_z_b, z_context_b);
	Wi_b->dot(true, dL_dpre_i_b, dL_dx_i_b, i_context_b);
	Wf_b->dot(true, dL_dpre_f_b, dL_dx_f_b, f_context_b);
	Wo_b->dot(true, dL_dpre_o_b, dL_dx, o_context_b);

	// dL_dW@ = dL_dpre_@ * x;
	dL_dpre_z_f->dot(sentence, dL_dWz_f, z_context_f);
	dL_dpre_i_f->dot(sentence, dL_dWi_f, i_context_f);
	dL_dpre_f_f->dot(sentence, dL_dWf_f, f_context_f);
	dL_dpre_o_f->dot(sentence, dL_dWo_f, o_context_f);

	dL_dpre_z_b->dot(sentence, dL_dWz_b, z_context_b);
	dL_dpre_i_b->dot(sentence, dL_dWi_b, i_context_b);
	dL_dpre_f_b->dot(sentence, dL_dWf_b, f_context_b);
	dL_dpre_o_b->dot(sentence, dL_dWo_b, o_context_b);

	// dL_dR@ = dL_dpre_@ * h;
	dL_dpre_z_f->dot(h, dL_dRz_f, z_context_f);
	dL_dpre_i_f->dot(h, dL_dRi_f, i_context_f);
	dL_dpre_f_f->dot(h, dL_dRf_f, f_context_f);
	dL_dpre_o_f->dot(h, dL_dRo_f, o_context_f);

	dL_dpre_z_b->dot(h, dL_dRz_b, z_context_b);
	dL_dpre_i_b->dot(h, dL_dRi_b, i_context_b);
	dL_dpre_f_b->dot(h, dL_dRf_b, f_context_b);
	dL_dpre_o_b->dot(h, dL_dRo_b, o_context_b);

	// dL_dp@ = c * dL_dpre_@;
	c_f->dot(dL_dpre_i_f, dL_dpi_f, z_context_f);
	c_f->dot(dL_dpre_f_f, dL_dpf_f, i_context_f);
	c_f->dot(dL_dpre_o_f, dL_dpo_f, o_context_f);

	c_b->hdot(dL_dpre_i_b, dL_dpi_b, z_context_b);
	c_b->hdot(dL_dpre_f_b, dL_dpf_b, i_context_b);
	c_b->hdot(dL_dpre_o_b, dL_dpo_b, o_context_b);
	...
	dL_dx->add(dL_dx_z_f, dL_dx_i_f, dL_dx_f_f, dL_dx_o_f, dL_dx_z_b, dL_dx_i_b, dL_dx_f_b, o_b_context);
