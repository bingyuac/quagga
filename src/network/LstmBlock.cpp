...
void LstmBlock::forwardPropagation(GpuMatrix *pre_z_t, GpuMatrix *pre_i_t, GpuMatrix *pre_f_t, GpuMatrix *pre_o_t, GpuMatrix *h_tm1, GpuMatrix *c_tm1, bool back_prop) {
	...
	pi->hprod(c_tm1, 1.0f, pre_i_t, i_context);
	if (back_prop) {
	...
	pf->hprod(c_tm1, 1.0f, pre_f_t, f_context);
	if (back_prop) {
	...
	GpuMatrix::sumHprod(i_t, z_t, f_t, c_tm1, c_t, c_context);
	...
	po->hprod(c_t, 1.0f, pre_o_t, o_context);
	if (back_prop) {
	...
	c_context->synchronize();
	tanh_c_t->hprod(o_t, 0.0f, h_t, o_context);
...
void LstmBlock::backwardPropagation(GpuMatrix *dL_dh_t, GpuMatrix *c_tm1) {
	...
	GpuMatrix::hprod(dL_dh_t, tanh_c_t, do_t_dpre_o_t, dL_dpre_o_t, o_context);

	// dL/dc[t] = dL/dh[t] .* o[t] .* dtanh(c[t])/dc[t] + po .* dL/dpre_o[t]
	GpuMatrix::sumHprod(dL_dh_t, o_t, dtanh_c_t_dc_t, po, dL_dpre_o_t, dL_dc_t, o_context);

	// dL/dpre_f[t] = dL/dc[t] .* c[t-1] .* df[t]/dpre_f[t]
	// dL/dpre_i[t] = dL/dc[t] .* z[t] .* di[t]/dpre_i[t]
	// dL/dpre_z[t] = dL/dc[t] .* i[t] .* dz[t]/dpre_z[t]
	o_context->synchronize();
	GpuMatrix::hprod(dL_dc_t, c_tm1, df_t_dpre_f_t, dL_dpre_f_t, f_context);
	GpuMatrix::hprod(dL_dc_t, z_t, di_t_dpre_i_t, dL_dpre_i_t, i_context);
	GpuMatrix::hprod(dL_dc_t, i_t, dz_t_dpre_z_t, dL_dpre_z_t, z_context);
}
...
void LstmBlock::backwardPropagation(GpuMatrix *dL_dpre_z_tp1, GpuMatrix *dL_dpre_i_tp1, GpuMatrix *dL_dpre_f_tp1, GpuMatrix *dL_dpre_o_tp1, GpuMatrix *dL_dc_tp1, GpuMatrix *f_tp1, GpuMatrix *c_tm1) {
	...
	Rz->tdot(dL_dpre_z_tp1, dL_dhz_t, z_context);
	Ri->tdot(dL_dpre_i_tp1, dL_dhi_t, i_context);
	Rf->tdot(dL_dpre_f_tp1, dL_dhf_t, f_context);
	Ro->tdot(dL_dpre_o_tp1, dL_dh_t, o_context);
	z_context->synchronize();
	i_context->synchronize();
	f_context->synchronize();
	...
	// dL/dpre_o[t] = dL/dh[t] .* tanh(c[t]) .* do[t]/dpre_o[t]
	GpuMatrix::hprod(dL_dh_t, tanh_c_t, dL_dpre_o_t, dL_dpre_o_t, o_context);

	// dL/dc[t] = dL/dh[t] .* o[t] .* dtanh(c[t])/dc[t] + po .* dL/dpre_o[t] +
	//													  pi .* dL/dpre_i[t+1] +
	//									    			  pf .* dL/dpre_f[t+1] +
	//											    	  dL/dc[t+1] .* f[t+1]
	GpuMatrix::sumHprod(dL_dh_t, o_t, dtanh_c_t_dc_t, po, dL_dpre_o_t, pi, dL_dpre_i_tp1, pf, dL_dpre_f_tp1, dL_dc_tp1, f_tp1, dL_dc_t, o_context);
