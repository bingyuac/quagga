...
from quagga.utils.initializers import Uniform
...
class DataBlock(object):
    def __init__(self, train_data, valid_data, batch_size, device_id):
    ...
    def fprop(self):
        ...
        for k, e in enumerate(data):
            enc_x_npa[k, :len(e)] = e
            dec_x_npa[k, :len(e) + 1] = [word_to_idx['<<S>>']] + e
...
if __name__ == '__main__':
    ...
    get_orth_W = Orthogonal(512, 1024)
    ...
    p = ParameterContainer(embd_W={'init': Uniform(len(word_to_idx) - 1, 512),
                                   'device_id': 0},
                           enc_lstm_c0={'init': Constant(1, 1024),
                                        'device_id': 0},
                           enc_lstm_h0={'init': Constant(1, 1024),
                                        'device_id': 0},
                           enc_lstm_W={'init': get_stacked_orth_W,
                                       'device_id': 0},
                           enc_lstm_R={'init': get_stacked_orth_R,
                                       'device_id': 0},
                           dec_lstm_c0={'init': Constant(1, 1024),
                                        'device_id': 0},
                           dec_lstm_W={'init': get_stacked_orth_W,
                                       'device_id': 0},
                           dec_lstm_R={'init': get_stacked_orth_R,
                                       'device_id': 0},
                           sce_dot_block_W={'init': Uniform(1024, len(word_to_idx)),
                                            'device_id': 0},
                           sce_dot_block_b={'init': Constant(1, len(word_to_idx)),
                                            'device_id': 0})
    data_block = DataBlock(train_data, valid_data, 64, device_id=0)
    ...
    # l2_reg_block = L2RegularizationBlock(last_selector_block.output, 0.0001)
    ...
    validator = Validator(model, 6000)
    validator.add_observer(valid_loss_tracker)
    saver = Hdf5Saver(p.trainable_parameters, 6000, 'auto.hdf5', logger)
