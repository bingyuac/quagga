...
class TestLogisticRegressionCe(TestCase):
    ...
    def test_theano_grad(self):
        ...
        for i in xrange(self.N):
            ...
            lr_cpu = LogisticRegressionCe(W_init, b_init, features, true_labels)
            lr_cpu.fprop()
            lr_cpu.bprop()
            lr_cpu.context.synchronize()
            context.synchronize()

            dL_dW_q = lr_cpu.dL_dW.to_host()
            dL_db_q = lr_cpu.dL_db.to_host()
            dL_dfeatures_q = lr_cpu.dL_dfeatures.to_host()
